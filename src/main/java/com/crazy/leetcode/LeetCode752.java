...
//        System.out.println(new LeetCode752().openLock(new String[]{"0000"}, "8888"));
    ...
    public int openLock(String[] deadends, String target) {
        ...
        String initStr = "0000";
        if (target.equals(initStr)) {
            return 0;
        }
        List<String> deadNumberList = Arrays.asList(deadends);
        if (deadNumberList.contains(initStr)) {
            return -1;
        }
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // 获取下一步可能出现的数字，已排除死亡数字
                List<String> nextNumbers = getNextNumbers(currentNumber, deadNumberList);
                if (nextNumbers.contains(target)) {
                    return minTimes + 1;
                }

                nextNumbers.forEach(queue::offer);
            }
    ...
    private List<String> getNextNumbers(String currentNumber, List<String> deadNumberList) {
        List<String> nextNumberList = new ArrayList<>();

        int[] currentNumber2Int = new int[currentNumber.length()];
        for (int i = 0; i < currentNumber.length(); i++) {
            currentNumber2Int[i] = currentNumber.charAt(i) - '0';
        }

        for (int i = 0; i < currentNumber.length(); i++) {
            StringBuilder stringBuilder = new StringBuilder(currentNumber);

            // 向上旋转
            int toUp = currentNumber2Int[i] - 1;
            toUp = toUp == -1 ? 9 : toUp;

            // 向下旋转
            int toDown = currentNumber2Int[i] + 1;
            toDown = toDown == 10 ? 0 : toDown;

            int[] possible = new int[]{toUp, toDown};

            for (int j = 0; j < possible.length; j++) {
                stringBuilder.setCharAt(i, (char) ('0' + possible[j]));
                if (!deadNumberList.contains(stringBuilder.toString())) {
                    nextNumberList.add(stringBuilder.toString());
                }
            }
        }
